...
static void keccak_inc_squeeze(uint8_t *h, size_t outlen,
    ...
    // 첫 번째 단계: 남은 바이트들
    size_t available = s_inc[25];
    if (available > 0 && outlen > 0) {
        size_t consume = (outlen < available) ? outlen : available;
        size_t start_pos = r - available;

        // 언롤링된 루프로 바이트 추출
        for (i = 0; i < consume; i++) {
            size_t pos = start_pos + i;
            h[i] = (uint8_t)(s_inc[pos >> 3] >> ((pos & 7) << 3));
        }
    ...
    while (outlen > 0) {
        ...
        size_t block_size = (outlen < r) ? outlen : r;

        // 64비트 단위로 빠른 복사
        const size_t words = block_size >> 3;
        __builtin_memcpy(h, s_inc, words << 3);

        // 마지막 부분적 워드 처리
        if (block_size & 7) {
            uint64_t partial = s_inc[words];
            for (i = words << 3; i < block_size; i++) {
                h[i] = (uint8_t)partial;
                partial >>= 8;
            }
        }
